...
import tpaga
from tpaga import error, http_client, version, util
from tpaga.multipart_data_generator import MultipartDataGenerator
...
def _api_encode(data):
    for key, value in data.iteritems():
        ...
        if value is None:
            continue
        elif hasattr(value, 'tpaga_id'):
            yield (key, value.tpaga_id)
...
class APIRequestor(object):
    ...
    def __init__(self, key=None, client=None, api_base=None, account=None):
        if api_base:
            ...
            self.api_base = tpaga.api_base
        self.api_key = key
        self.tpaga_account = account

        from tpaga import verify_ssl_certs
    ...
    def api_url(cls, url=''):
        warnings.warn(
            ...
            'at support@tpaga.com.',
            DeprecationWarning)
        return '%s%s' % (tpaga.api_base, url)
    ...
    def request_raw(self, method, url, params=None, supplied_headers=None):
        ...
        """
        from tpaga import api_version

        if self.api_key:
            my_api_key = self.api_key
        else:
            from tpaga import api_key
            my_api_key = api_key

        if my_api_key is None:
            raise error.AuthenticationError(
                'No API key provided. (HINT: set your API key using '
                '"tpaga.api_key = <API-KEY>"). You can generate API keys '
                'from the tpaga web interface.  See https://tpaga.com/api '
                'for details, or email support@tpaga.com if you have any '
                'questions.')
        ...
        if method == 'get' or method == 'delete':
            ...
            raise error.APIConnectionError(
                ...
                'tpaga bindings.  Please contact support@tpaga.com for '
                'assistance.' % (method,))
        ...
        headers = {
            'X-tpaga-Client-User-Agent': util.json.dumps(ua),
            'User-Agent': 'tpaga/v1 PythonBindings/%s' % (version.VERSION,),
            'Authorization': 'Bearer %s' % (my_api_key,)
        }

        if self.tpaga_account:
            headers['tpaga-Account'] = self.tpaga_account

        if method == 'post':
            headers['Content-Type'] = 'application/x-www-form-urlencoded'

        if api_version is not None:
            headers['tpaga-Version'] = api_version
    ...
    def requests_request(self, meth, abs_url, headers, params):
        from tpaga.http_client import RequestsClient
    ...
    def pycurl_request(self, meth, abs_url, headers, params):
        from tpaga.http_client import PycurlClient
    ...
    def urlfetch_request(self, meth, abs_url, headers, params):
        from tpaga.http_client import UrlFetchClient
    ...
    def urllib2_request(self, meth, abs_url, headers, params):
        from tpaga.http_client import Urllib2Client
